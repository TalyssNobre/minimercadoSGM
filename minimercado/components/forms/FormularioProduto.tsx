...
export default function FormularioProduto() {
  ...
  // Limpa o preview da memória ao sair da tela
  useEffect(() => {
    return () => { if (previewUrl) URL.revokeObjectURL(previewUrl); };
  }, [previewUrl]);

  // Constante para não repetir o CSS gigante em todo input
    ...
    <>
      <form onSubmit={handleSalvarProduto} className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <ButtonSistema type="button" onClick={() => setIsModalCategoriaOpen(true)} className="px-4 py-2.5 text-xs whitespace-nowrap capitalize flex-shrink-0">
                + Nova Categoria
              </ButtonSistema>
            </div>
          </div>
        ...
        <div>
          ...
          <div className="w-full aspect-video border-2 border-dashed border-gray-300 bg-gray-50/50 rounded-lg flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 transition-colors relative group overflow-hidden">
            <input type="file" accept="image/*" onChange={handleImageChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
            
            {previewUrl ? (
              <div className="relative w-full h-full">
                <img src={previewUrl} alt="Preview" className="w-full h-full object-cover" />
                <button type="button" onClick={handleRemoveImage} className="absolute top-3 right-3 z-20 bg-red-600 text-white rounded-full p-2 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-700" title="Remover imagem">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              </div>
            ) : (
              <div className="flex flex-col items-center text-gray-400 group-hover:text-[#0D9488] transition-colors p-6 text-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-10 h-10 mb-3"><path d="M12 9a3.75 3.75 0 100 7.5A3.75 3.75 0 0012 9z" /><path fillRule="evenodd" d="M9.344 3.071a49.52 49.52 0 015.312 0c.967.052 1.83.585 2.332 1.39l.821 1.317c.24.383.645.643 1.11.71.386.054.77.113 1.152.177 1.432.239 2.429 1.493 2.429 2.909V18a3 3 0 01-3 3h-15a3 3 0 01-3-3V9.574c0-1.416.997-2.67 2.429-2.909.382-.064.766-.123 1.151-.178a1.56 1.56 0 001.11-.71l.822-1.315a2.942 2.942 0 012.332-1.39zM6.75 12.75a5.25 5.25 0 1110.5 0 5.25 5.25 0 01-10.5 0z" clipRule="evenodd" /></svg>
                <span className="text-sm font-medium">Clique ou arraste uma imagem do produto</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-4 pt-4">
          <ButtonSistema type="button" variant="outline">Cancelar</ButtonSistema>
          <ButtonSistema type="submit" variant="primary">Salvar Produto</ButtonSistema>
        </div>
      </form>
      ...
      {isModalCategoriaOpen && (
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center backdrop-blur-sm p-4">
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-md overflow-hidden">
            <form onSubmit={handleSalvarCategoria} className="p-8 space-y-6">
